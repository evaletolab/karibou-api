...
Product.statics.findPopularByUser = function(criteria, callback){
  ...
  var skus=[], today=new Date(), windowtime=(parseInt(criteria.windowtime)||2)-1, year=today.getFullYear();
  ...
  this.model('Orders').aggregate(
     ...
     { $match: { 'month': {$gt:today.getMonth()-windowtime,$lte:today.getMonth()+1 }, 'year':year } },     
     {$unwind: '$items' },
     {$group:
       {
         _id:"$items.sku",
