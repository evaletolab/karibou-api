...
exports.execute = function(db, script, callback){
	console.log(script,"Convert all products.categories to products.category");
  var logs="", count=0;
  var products=db.collection('products');

  products.find( {$where : "Array.isArray(this.categories)"}).toArray(function (err,p) {
	  if (!p.length){
    ...
    console.log(script,"migrating "+p.length +" products");
      ...
      products.save(product,function(err){
        console.log(err, product.categories)
        eachcb(err);        
      });
...
//      });
	  ...
	  function(err){
        return callback(err, p.length+" categories on products have been updated");
	  });
  });	
