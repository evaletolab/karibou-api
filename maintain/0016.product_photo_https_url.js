...
exports.execute = function(db, script, callback){
	console.log(script,"Convert all products.photo.url to be https complient");
  var logs="", count=0;
  ...
  products.find( {}).toArray(function (err,p) {
    ...
    require('async').each(p, function(product, eachcb){
      ...
      if(!product.photo||!product.photo.url){
	    ...
	    var url=product.photo.url.split(aws.in);
      if(url.length>1){
        product.photo.url=aws.out+url[1]
      }
      console.log('rename',product.photo.url )
